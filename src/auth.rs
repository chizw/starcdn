    ...
    headers
        ...
        .and_then(|cookie| {
            cookie
                .split(';')
                .map(str::trim)
                .find_map(|part| part.strip_prefix("admin_token=").map(str::to_string))
        })
}
...
pub async fn login(
    ...
    let token = token_for(id, &username, &state.config.jwt_secret)
        .map_err(|_| json_error(StatusCode::INTERNAL_SERVER_ERROR, "token error"))?;
    let mut headers = HeaderMap::new();
    headers.insert(
        header::SET_COOKIE,
        format!("admin_token={}; Path=/; Max-Age=86400; SameSite=Lax", token)
            .parse()
            .unwrap(),
    );
...
pub async fn change_password(
    ...
    let new_hash = hash_password(&req.new_password)
        .map_err(|_| json_error(StatusCode::INTERNAL_SERVER_ERROR, "hash error"))?;
