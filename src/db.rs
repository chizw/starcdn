...
impl Database {
    pub async fn connect(config: &Config) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        database.ensure_admin(&config.admin_user, &config.admin_pass).await?;
        Ok(database)
    }
    ...
    async fn migrate(&self, reset: bool) -> Result<(), sqlx::Error> {
        let legacy = sqlx::query("SELECT name FROM sqlite_master WHERE type='table' AND name='users'").fetch_optional(&self.pool).await?.is_some()
            && sqlx::query("PRAGMA table_info(users)").fetch_all(&self.pool).await?.iter().any(|row| row.get::<String, _>("name") == "passkey_credentials_json");
        if legacy && !reset {
        ...
        if reset {
            for table in ["users", "sessions", "traffic_stats", "ban_rules", "cache_purge_logs", "schema_migrations"] {
    ...
    async fn ensure_admin(&self, username: &str, password: &str) -> Result<(), sqlx::Error> {
        let exists = sqlx::query("SELECT id FROM users WHERE username = ?").bind(username).fetch_optional(&self.pool).await?.is_some();
        if !exists {
            let now = Utc::now().to_rfc3339();
            let hash = crate::auth::hash_password(password).map_err(|err| sqlx::Error::Protocol(err))?;
    ...
    pub async fn stats(&self, page: i64, page_size: i64) -> Result<StatsSummary, sqlx::Error> {
        ...
        let top_urls = rows.into_iter().map(|row| TopUrl { request_path: row.get("request_path"), request_count: row.get("request_count"), bytes_sent: row.get("bytes_sent") }).collect();
        Ok(StatsSummary { total_requests: total.get("requests"), total_bytes_sent: total.get("bytes"), unique_paths, total_pages, current_page: page, page_size, top_urls })
    }
    ...
    pub async fn list_bans(&self) -> Result<Vec<BanRuleRecord>, sqlx::Error> {
        ...
        Ok(rows.into_iter().map(|row| BanRuleRecord { id: row.get("id"), pattern: row.get("pattern"), mode: row.get("mode"), reason: row.get("reason"), enabled: row.get::<i64, _>("enabled") != 0, created_at: row.get("created_at"), updated_at: row.get("updated_at") }).collect())
    }

    pub async fn create_ban(&self, pattern: &str, mode: &str, reason: &str) -> Result<BanRuleRecord, sqlx::Error> {
        ...
        Ok(BanRuleRecord { id, pattern: pattern.to_string(), mode: mode.to_string(), reason: reason.to_string(), enabled: true, created_at: now.clone(), updated_at: now })
    }

    pub async fn delete_ban(&self, id: i64) -> Result<(), sqlx::Error> {
        sqlx::query("DELETE FROM ban_rules WHERE id = ?").bind(id).execute(&self.pool).await?;
        Ok(())
    }
}
